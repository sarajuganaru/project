...
derivatives = os.path.join(data, 'derivatives')
participants = os.path.join(data, 'participants.tsv')
# %% frequency bands
freq_bands = {
    'delta': (0.5, 4), #hpf
    'theta': (4, 8),
...
cm = confusion_matrix(all_true, all_preds, labels=['A','F','C'])
...
disp = ConfusionMatrixDisplay(
    confusion_matrix=cm,
    display_labels=['AD','FTD','Control']
)

disp.plot(cmap='Blues')
plt.title("Confusion Matrix (LOSO KNN)")
plt.show()
